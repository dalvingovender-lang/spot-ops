...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (!meeting || !meeting.id) { res.status(400).json({ error: 'Missing id' }); return; }
      const { blobs } = await list({ token, prefix: 'meetings/' });
      const existing = blobs.filter(b => b.pathname.includes(meeting.id));
      if (existing.length > 0) await Promise.all(existing.map(b => del(b.url, { token })));
      await put(`meetings/${meeting.id}.json`, JSON.stringify(meeting), { access: 'public', token, contentType: 'application/json' });
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      if (toDelete.length > 0) await Promise.all(toDelete.map(b => del(b.url, { token })));
      res.status(200).json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
    return;
  }
